...
The allocation adjustment based on distance follows:
\[
a_{sr}^{\tau}(t+1) = \begin{cases}
a_{sr}^{\tau}(t) - \alpha |d_{sr}(t)| & \text{if } \Delta_r^\tau(t) < 0 \text{ (retract from overshoot)} \\
a_{sr}^{\tau}(t) + \alpha |d_{sr}(t)| & \text{if } \Delta_r^\tau(t) > 0 \text{ (expand toward undershoot)} \\
a_{sr}^{\tau}(t) - \alpha \cdot d_{sr}(t) & \text{if } \Delta_r^\tau(t) = 0 \text{ (rebalance for alignment)}
\end{cases}
\]
where:
\begin{itemize}
\item $d_{sr}(t) = a_{sr}^\tau(t) - t_{sr}^\tau$ is the deviation from target (computed using Theorem \ref{thm:dual-priority-flow})
\item $\alpha \in (0,1]$ is the damping factor (typically 0.8)
\item $t$ is the discrete iteration number
\end{itemize}

This implements discrete-time gradient descent on the energy landscape:
\[
E(t) = \frac{1}{2} \sum_{r,\tau} (\Delta_r^\tau(t))^2
\]

At each iteration, energy decreases:
\[
E(t+1) \leq (1-\alpha)^2 E(t)
\]
\end{definition}
...
\begin{proof}
Each provider independently computes targets using Theorem \ref{thm:dual-priority-flow} and applies full utilization (Theorem \ref{thm:full-utilization-flow}). The distance-based adjustment ensures convergence to targets. 
...
By the Banach fixed-point theorem, this converges to the unique fixed point. At equilibrium, $a_{sr}^\tau = t_{sr}^\tau$ for all providers, which implies:
\begin{itemize}
\item If $\sum_s q_s \geq \sum_r N_r^\tau$: sufficient capacity exists, so $\Delta_r^\tau = 0$ for all $r$
\item If $\sum_r N_r^\tau \geq \sum_s q_s$: sufficient need exists, so full utilization ensures $\sum_r a_{sr}^\tau = q_s$ for all $s$
\item All allocations respect both provider and recipient priorities via the $\min$ operator
\end{itemize}
\qed
\end{proof}
...
\begin{definition}[Iterative Allocation Algorithm]
\label{alg:iterative-convergence}

\textbf{Input}: 
\begin{itemize}
\item Sources $S \subseteq \mathcal{N}$ with potentials $\{p_s = (\tau, q_s, C_s) \mid q_s > 0\}$
\item Recipients $R \subseteq \mathcal{N}$ with potentials $\{p_r = (\tau, q_r, C_r) \mid q_r < 0\}$
\item Provider weights $\{w_s: R \to \mathbb{R}_{\geq 0}\}_{s \in S}$ with $\sum_{r \in R} w_s(r) = 1$
\item Recipient prioritizations $\{G_r: S \to \mathbb{R}_{\geq 0}\}_{r \in R}$ with $\sum_{s \in S} G_r(s) = 1$
\item Damping factor $\alpha \in (0,1]$ (typically 0.8)
\item Convergence threshold $\epsilon > 0$ (typically $10^{-6}$)
\end{itemize}

\textbf{Output}: Allocations $\{a_{sr}^\tau\}$ at equilibrium

\textbf{Procedure}:
\begin{enumerate}
\item \textbf{Initialize}: Set $a_{sr}^\tau \leftarrow 0$ for all $s \in S, r \in R, \tau$

\item \textbf{Repeat until convergence}:
  \begin{enumerate}
  \item \textbf{Compute targets} for each provider $s$ and recipient $r$:
  \begin{align*}
  E_{sr}^\tau &\leftarrow |q_r| \cdot G_r(s) \quad \text{(recipient expectation)} \\
  t_{sr}^\tau &\leftarrow \min\left( q_s \cdot \frac{w_s(r)}{\sum_{r' \in R} w_s(r')}, \, E_{sr}^\tau, \, C(r) \right)
  \end{align*}
  
  \item \textbf{Apply remainder redistribution} (Theorem \ref{thm:remainder-redistribution}):
  \begin{align*}
  R_s &\leftarrow q_s - \sum_{r \in R} t_{sr}^\tau \\
  R_{\text{eligible}} &\leftarrow \{ r \in R \mid \sum_{s'} t_{s'r} < |q_r| \land C(r) > \sum_{s'} t_{s'r} \} \\
  \text{For } r \in R_{\text{eligible}}&: \quad t_{sr}^\tau \leftarrow t_{sr}^\tau + \min\left( R_s \cdot \frac{w_s(r)}{\sum_{r' \in R_{\text{eligible}}} w_s(r')}, |q_r| - \sum_{s'} t_{s'r} \right)
  \end{align*}
  
  \item \textbf{Compute distances} for each recipient $r$:
  \begin{align*}
  A_r^\tau &\leftarrow \sum_{s \in S} a_{sr}^\tau \\
  \Delta_r^\tau &\leftarrow |q_r| - A_r^\tau
  \end{align*}
  
  \item \textbf{Adjust allocations} for each provider $s$ and recipient $r$:
  \begin{align*}
  d_{sr} &\leftarrow a_{sr}^\tau - t_{sr}^\tau \\
  a_{sr}^\tau &\leftarrow \begin{cases}
  a_{sr}^\tau - \alpha |d_{sr}| & \text{if } \Delta_r^\tau < 0 \text{ (retract from overshoot)} \\
  a_{sr}^\tau + \alpha |d_{sr}| & \text{if } \Delta_r^\tau > 0 \text{ (expand toward undershoot)} \\
  a_{sr}^\tau - \alpha \cdot d_{sr} & \text{if } \Delta_r^\tau = 0 \text{ (rebalance for alignment)}
  \end{cases}
  \end{align*}
  
  \item \textbf{Check convergence}:
  \[
  \text{if } \max_{r \in R, \tau} |\Delta_r^\tau| < \epsilon \text{ then break}
  \]
  \end{enumerate}

\item \textbf{Return}: $\{a_{sr}^\tau\}$
\end{enumerate}
\end{definition}
...
The number of iterations to reach error $\epsilon$ from initial distance $\|\Delta_0\|$ is:
\[
T_\epsilon = O\left(\frac{\log(\epsilon / \|\Delta_0\|)}{\log(1-\alpha)}\right)
\]

For $\alpha = 0.8$, error reduces by 99.99\% in approximately 6 iterations.
\end{theorem}

\begin{proof}
The distance-based adjustment implements gradient descent on the energy landscape:
\[
E = \frac{1}{2} \sum_{r \in R, \tau} (\Delta_r^\tau)^2
\]

Each iteration reduces this energy by factor $(1-\alpha)$:
\[
E(t+1) \leq (1-\alpha)^2 E(t)
\]

Therefore $\|\Delta(t+1)\|_F = \sqrt{2E(t+1)} \leq (1-\alpha) \sqrt{2E(t)} = (1-\alpha) \|\Delta(t)\|_F$.

Setting $\|\Delta(T)\|_F \leq \epsilon$ and solving:
\[
(1-\alpha)^T \|\Delta_0\|_F \leq \epsilon \implies T \geq \frac{\log(\epsilon / \|\Delta_0\|_F)}{\log(1-\alpha)}
\]

For $\alpha = 0.8$: $(1-0.8)^6 = 0.000064 \approx 0.0001$, giving 99.99\% error reduction. \qed
\end{proof}

\begin{proposition}[Damping Factor Trade-offs]
\label{prop:damping-tradeoffs}
The damping factor $\alpha \in (0,1]$ controls the convergence-stability trade-off:

\begin{center}
\begin{tabular}{|c|c|c|l|}
\hline
\textbf{$\alpha$} & \textbf{Iterations to 99.99\%} & \textbf{Stability} & \textbf{Use Case} \\
\hline
0.1 & $\approx 22$ & Very High & Highly dynamic networks \\
0.5 & $\approx 8$ & High & Moderate dynamics \\
0.8 & $\approx 6$ & Good & Most scenarios (recommended) \\
0.9 & $\approx 4$ & Moderate & Static networks \\
0.99 & $\approx 2$ & Low & Rare, oscillation risk \\
\hline
\end{tabular}
\end{center}

\textbf{Optimal range}: $\alpha \in [0.7, 0.9]$ balances fast convergence with stability.
\end{proposition}

\begin{proof}
From Theorem \ref{thm:convergence-rate}, iterations to reach error $\epsilon$ is $T_\epsilon = \log(\epsilon / \|\Delta_0\|) / \log(1-\alpha)$.

For 99.99\% error reduction ($\epsilon / \|\Delta_0\| = 0.0001$):
\begin{itemize}
\item $\alpha = 0.1$: $T \approx \log(0.0001) / \log(0.9) \approx -9.21 / -0.105 \approx 22$
\item $\alpha = 0.5$: $T \approx -9.21 / -0.693 \approx 8$
\item $\alpha = 0.8$: $T \approx -9.21 / -1.609 \approx 6$
\item $\alpha = 0.9$: $T \approx -9.21 / -2.303 \approx 4$
\end{itemize}

As $\alpha$ increases, convergence accelerates, but larger adjustment steps increase oscillation risk when distance estimates are noisy. \qed
\end{proof}

\begin{remark}[Practical Guidance]
In practice, $\alpha = 0.8$ works well across diverse scenarios. For highly dynamic networks where needs/capacities change frequently, reduce to $\alpha = 0.5$ for better stability. For static networks with infrequent updates, increase to $\alpha = 0.9$ for faster convergence.
\end{remark}

\subsection{Robustness to Network Partitions}

\begin{proposition}[Self-Correction via Negative Feedback]
When network partitions create inconsistent views of $A_r^\tau$, the system self-corrects through negative feedback:

\textbf{Temporary over-allocation} ($\Delta_r^\tau < 0$): All providers retract allocations proportionally

\textbf{Temporary under-allocation} ($\Delta_r^\tau > 0$): All providers expand allocations proportionally

Errors decay geometrically with rate $(1-\alpha)^t$, ensuring rapid convergence once partitions heal.
\end{proposition}

\begin{proof}
The distance-based adjustment (step 2d in Definition \ref{alg:iterative-convergence}) creates a negative feedback loop. When $\Delta_r^\tau < 0$ (over-allocation), all providers reduce $a_{sr}^\tau$ toward targets. When $\Delta_r^\tau > 0$ (under-allocation), all providers increase $a_{sr}^\tau$ toward targets. By Theorem \ref{thm:convergence-rate}, errors decay at rate $(1-\alpha)^t$. \qed
\end{proof}
...
Suppose providers observe stale distances with bounded staleness:
\[
|\Delta_r^\tau(\text{observed}) - \Delta_r^\tau(\text{actual})| \leq \varepsilon_{\text{stale}}
\]

Then the system still converges, with convergence time:
\[
T_\varepsilon = O\left(\frac{\log(\varepsilon / \|\Delta_0\|)}{\log(1-\alpha)} + \frac{\varepsilon_{\text{stale}}}{\alpha \cdot \min_s q_s}\right)
\]

Staleness adds a constant delay but doesn't prevent convergence.
\end{proposition}
...
\begin{definition}[Damping for Stability]
To prevent oscillations in allocation, introduce a damping factor $\delta(r) \in \{0.5, 0.8, 1.0\}$ based on oscillation detection. The \textbf{active need} becomes:
\[
q_{\text{active}}(r) = q_{\text{declared}}(r) \times \delta(r)
\]
where $\delta(r)$ decreases when oscillations are detected in $r$'s allocation history.
\end{definition}

\begin{proposition}[Damping Preserves Convergence]
Damping factors $\delta \in (0, 1]$ reduce the fill fraction but preserve the convergence property:
\[
\|r(t)\| \leq (1-f')^t \cdot \|r(0)\|
\]
where $f' = f \cdot \min_r \delta(r) > 0$. The system still converges, though potentially at a slower rate.
\end{proposition}

\begin{proof}
Damping reduces active needs to $q_{\text{active}}(r) = \delta(r) \cdot q_{\text{declared}}(r)$. The fill fraction becomes $f' = f \cdot \min_r \delta(r)$ since less need is expressed. Since $\delta(r) \in (0, 1]$, we have $f' > 0$, and the convergence argument from Theorem 13 applies with $f'$ replacing $f$. \qed
\end{proof}
