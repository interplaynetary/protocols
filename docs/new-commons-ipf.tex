...
\author{Rüzgar Imski and Free-Association Research Community}
\date{}
...
\begin{remark}[Distributed Implementation]
In the distributed implementation, where agents coordinate asynchronously by exchanging scaling factors, proposals are computed directly as $A_{pr} = K_{pr} \cdot x_p \cdot y_r$ without iterating through the full Sinkhorn loop. Since providers and recipients update their scaling factors independently and asynchronously, the product $K_{pr} \cdot x_p \cdot y_r$ may temporarily exceed the recipient's need $N_r$ before the system converges. To ensure the need constraint is always respected, proposals are explicitly clamped:
\[ A_{pr} = \min(K_{pr} \cdot x_p \cdot y_r, N_r) \]
In contrast, the centralized algorithm enforces this constraint through iterative column scaling (lines 150-167 in the implementation), which scales down all allocations to a recipient when their column sum exceeds their need. Both approaches ensure $\sum_p A_{pr} \le N_r$, but the distributed version requires the explicit clamp because it computes final proposals from potentially stale scaling factors rather than iterating to convergence.
\end{remark}
