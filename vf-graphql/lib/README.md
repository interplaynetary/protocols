...
If all you need is the _entire_ schema as a string, consider importing `@valueflows/vf-graphql/ALL_VF_SDL` or `@valueflows/vf-graphql/json-schema.json` instead.
...
- **Bun**: This project now uses Bun as the package manager. Install it from [bun.sh](https://bun.sh)
    ```bash
...
1. Run `bun install` from the top level folder of this repository to install and wire up all dependencies.
2. Run `bun run build` to compile the schema files.

### Available commands

See `scripts` in `package.json` for the available commands. For quickly spinning up the full system, you should usually be able to simply run `bun start`. This will load up:
...
- A GraphQL query UI at `http://localhost:3000/graphql` which you can use to test queries against a mock GraphQL API derived from the schema.
...
This repository now includes an MCP (Model Context Protocol) server that exposes the ValueFlows GraphQL schema for AI model integration. See [`mcp-server/README.md`](mcp-server/README.md) for details on:

- Running the MCP server
- Connecting it to Claude Desktop or other MCP clients
- Available tools and resources for AI-powered schema exploration
