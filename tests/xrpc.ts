...
// ─── Known lexicons ───────────────────────────────────────────────────────────
// These reflect what is uploaded from lexicons/vf/ via `bun hv.ts lexicons sync --only vf`
...
const QUERY_ENDPOINTS = [
  // Knowledge
  { nsid: "vf.knowledge.listActions",                collection: "vf.knowledge.action" },
  { nsid: "vf.knowledge.listProcessSpecifications",  collection: "vf.knowledge.processSpecification" },
  { nsid: "vf.knowledge.listRecipeExchanges",        collection: "vf.knowledge.recipeExchange" },
  { nsid: "vf.knowledge.listRecipeFlows",            collection: "vf.knowledge.recipeFlow" },
  { nsid: "vf.knowledge.listRecipeProcesses",        collection: "vf.knowledge.recipeProcess" },
  { nsid: "vf.knowledge.listRecipes",                collection: "vf.knowledge.recipe" },
  { nsid: "vf.knowledge.listResourceSpecifications", collection: "vf.knowledge.resourceSpecification" },
  { nsid: "vf.knowledge.listSpatialThings",          collection: "vf.knowledge.spatialThing" },
  { nsid: "vf.knowledge.listUnits",                  collection: "vf.knowledge.unit" },
  // Observation
  { nsid: "vf.observation.listAgents",               collection: "vf.observation.agent" },
  { nsid: "vf.observation.listBatchLotRecords",      collection: "vf.observation.batchLotRecord" },
  { nsid: "vf.observation.listEcologicalAgents",     collection: "vf.observation.ecologicalAgent" },
  { nsid: "vf.observation.listEconomicEvents",       collection: "vf.observation.economicEvent" },
  { nsid: "vf.observation.listEconomicResources",    collection: "vf.observation.economicResource" },
  { nsid: "vf.observation.listOrganizations",        collection: "vf.observation.organization" },
  { nsid: "vf.observation.listPersons",              collection: "vf.observation.person" },
  // Planning
  { nsid: "vf.planning.listAgreementBundles",        collection: "vf.planning.agreementBundle" },
  { nsid: "vf.planning.listAgreements",              collection: "vf.planning.agreement" },
  { nsid: "vf.planning.listClaims",                  collection: "vf.planning.claim" },
  { nsid: "vf.planning.listCommitments",             collection: "vf.planning.commitment" },
  { nsid: "vf.planning.listIntents",                 collection: "vf.planning.intent" },
  { nsid: "vf.planning.listPlans",                   collection: "vf.planning.plan" },
  { nsid: "vf.planning.listProcesses",               collection: "vf.planning.process" },
  { nsid: "vf.planning.listProposalLists",           collection: "vf.planning.proposalList" },
  { nsid: "vf.planning.listProposals",               collection: "vf.planning.proposal" },
] as const;
...
describe("query parameters", () => {
  ...
  const testNsid = "vf.knowledge.listUnits";
  ...
  // Repeat uri/did tests for each namespace's primary endpoint
  test("vf.observation.listPersons: uri fetch works", async () => {
    const nsid = "vf.observation.listPersons";
  ...
  test("vf.observation.listEconomicEvents: uri fetch works", async () => {
    const nsid = "vf.observation.listEconomicEvents";
    const list = await fetch(`${BASE_URL}/xrpc/${nsid}?limit=1`).then((r) => r.json());
    if (list.records.length === 0) return;
  ...
  test("vf.planning.listIntents: uri fetch works", async () => {
    const nsid = "vf.planning.listIntents";
    const list = await fetch(`${BASE_URL}/xrpc/${nsid}?limit=1`).then((r) => r.json());
    if (list.records.length === 0) return;
...
describe("record schema validation", () => {
  test("vf.knowledge.action records have required actionId and label", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.knowledge.listActions?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
      if (record.value) {
  ...
  test("vf.knowledge.unit records have required label", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.knowledge.listUnits?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
      if (record.value) {
        expect(typeof record.value.label).toBe("string");
  ...
  test("vf.observation.economicEvent records have required action field", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.observation.listEconomicEvents?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
      if (record.value) {
        expect(typeof record.value.action).toBe("string");
        // action must be an AT-URI pointing to a vf.knowledge.action record
        expect(record.value.action).toMatch(/^at:\/\//);
      }
    }
  });

  test("vf.planning.commitment records have required action field", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.planning.listCommitments?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
      if (record.value) {
        expect(typeof record.value.action).toBe("string");
  ...
  test("vf.planning.intent records have required action field", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.planning.listIntents?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
      if (record.value) {
        expect(typeof record.value.action).toBe("string");
  ...
  test("vf.observation.agent records have valid type enum value", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.observation.listAgents?limit=10`);
    const data = await res.json();
  ...
  test("vf.observation.economicResource accountingQuantity is a Measure object", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.observation.listEconomicResources?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
  ...
  test("vf.planning.agreement stipulates is an array of AT-URIs", async () => {
    const res = await fetch(`${BASE_URL}/xrpc/vf.planning.listAgreements?limit=10`);
    const data = await res.json();
    for (const record of data.records) {
