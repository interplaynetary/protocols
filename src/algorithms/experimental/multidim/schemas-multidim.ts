...
export const AvailabilitySlotSchema = z.object({
	...
	type_id: z.string().min(1), // k in C_j^k(t)
...
export const NeedSlotSchema = z.object({
	...
	// REQUIRED: Type specification for multi-dimensional framework
	type_id: z.string().min(1), // k in N_i^k(t)
...
export const SlotAllocationRecordSchema = z.object({
	...
	// ADJUSTMENT SOURCE (New for Stage 3)
	adjustment_source: z.enum(['mr_weighted', 'normal_mr', 'none']).optional()
});

export type SlotAllocationRecord = z.infer<typeof SlotAllocationRecordSchema>;

// ═══════════════════════════════════════════════════════════════════
// ADJUSTMENT SUGGESTIONS (New for Stage 3)
// ═══════════════════════════════════════════════════════════════════

/**
 * Provider Adjustment Suggestion
 * Details how a specific provider should adjust to correct overshoot
 */
export const ProviderAdjustmentSuggestionSchema = z.object({
	suggested_allocation: z.number(),
	mr_share: z.number(),
	responsibility_score: z.number(),
	ideal_allocation: z.number()
});

/**
 * Slot Adjustment Suggestion
 * Contains overshoot amount and suggested adjustments for all providers
 */
export const AdjustmentSuggestionSchema = z.object({
	overshoot_amount: z.number(),
	provider_adjustments: z.record(z.string(), ProviderAdjustmentSuggestionSchema) // provider_pubkey -> suggestion
});

export type ProviderAdjustmentSuggestion = z.infer<typeof ProviderAdjustmentSuggestionSchema>;
...
export const CommitmentSchema = z.object({
	...
	// NEW: Adjustment suggestions for overshot slots (Stage 3)
	// Format: { slot_id: adjustment_data }
	adjustment_suggestions: z.record(z.string(), AdjustmentSuggestionSchema).optional(),
...
export const TwoTierAllocationStateSchema = z.object({
	...
	slot_denominators: z.record(
		...
		z.object({
			...
			type_id: z.string().min(1) // Required: type per slot
		})
	),
...
export const MultiDimensionalNeedStateSchema = z.object({
	...
	needsByType: z.record(z.string(), PerTypeNeedStateSchema), // type_id -> state
	timestamp: z.number().int().positive(),
...
export const MultiDimensionalCapacityStateSchema = z.object({
	pubKey: z.string().min(1),
	capacitiesByType: z.record(z.string(), PerTypeCapacityStateSchema), // type_id -> state
	timestamp: z.number().int().positive(),
...
// ═══════════════════════════════════════════════════════════════════
// ALGORITHM TYPES (Moving from allocation.ts to schemas for sharing)
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
// OPTIMIZATION & STORAGE TYPES (From Complexity Analysis)
// ═══════════════════════════════════════════════════════════════════

/**
 * Provider State for Incremental Updates
 * Reduces O(N^2) by caching previous states
 */
export const ProviderStateSchema = z.object({
	// Per provider slot
	slots: z.map(z.string(), z.object({
		capacity: z.number(),
		allocations: z.map(z.string(), z.object({
			target: z.number(),
			mr_share: z.number(),
	...
	recipient_states: z.map(z.string(), z.object({
		total_allocated_by_slot: z.map(z.string(), z.number()),
		adjustment_suggestions: z.map(z.string(), AdjustmentSuggestionSchema).optional()
	}))
});

/**
 * Recipient State for Incremental Updates
 */
export const RecipientStateSchema = z.object({
	// Per slot
	slots: z.map(z.string(), z.object({
		declared_need: z.number(),
		total_allocated: z.map(z.string(), z.number()), // provider_id -> quantity
		adjustment_history: z.array(AdjustmentSuggestionSchema),
		mr_shares: z.map(z.string(), z.number()) // From Stage 1
	})),

	// Provider MR cache
	provider_mr: z.map(z.string(), z.number())
});
