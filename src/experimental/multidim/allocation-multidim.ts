...
export function calculateBidirectionalMRDistribution(
    ...
    for (const slot of providerSlots) {
        if (!providerSlotsByType.has(slot.need_type_id)) {
            providerSlotsByType.set(slot.need_type_id, []);
        }
        providerSlotsByType.get(slot.need_type_id)!.push(slot);
    }
    ...
    for (const [recipientPubKey, commitment] of Object.entries(allCommitments)) {
        ...
        for (const rSlot of recipientNeedSlots) {
            ...
            const potentialMatches = providerSlotsByType.get(rSlot.need_type_id) || [];
    ...
    for (const [key, target] of targets.entries()) {
        ...
        if (finalQuantity > 0) {
            allocations.push({
                ...
                need_type_id: providerSlot.need_type_id,
